...
SAMPLE_WORKS = [
    (1, "WO-0001", 2, "Ремонт компьютера", 1, 1500.0, 1500.0, 1,
     "Иванов И.И.", "2026-01-10", "completed", "Замена оперативной памяти"),
    (2, "WO-0002", 8, "Чистка от пыли", 1, 1000.0, 1000.0, 2,
     "Петрова А.С.", "2026-01-15", "completed", "Ноутбук перегревался"),
    (3, "WO-0003", 6, "Установка Windows", 1, 2500.0, 2500.0, 3,
     "Сидоров К.В.", "2026-01-22", "completed", "Windows 11, драйверы"),
    (4, "WO-0004", 4, "Замена экрана", 1, 3500.0, 3500.0, 4,
     "Козлова М.П.", "2026-02-03", "completed", "Матрица 15.6"),
    (5, "WO-0005", 1, "Диагностика оборудования", 2, 500.0, 1000.0, 5,
     "ООО ТехноСервис", "2026-02-18", "completed", "Два устройства"),
    (6, "WO-0006", 11, "Настройка сети", 1, 1200.0, 1200.0, 5,
     "ООО ТехноСервис", "2026-03-01", "completed", "Wi-Fi роутер и принтер"),
    (7, "WO-0007", 10, "Восстановление данных", 1, 3000.0, 3000.0, 6,
     "Белова Е.Н.", "2026-03-12", "completed", "Восстановлены документы"),
    (8, "WO-0008", 3, "Ремонт ноутбука", 1, 2000.0, 2000.0, 7,
     "Федоров П.Г.", "2026-03-25", "in_progress", "Диагностика материнской платы"),
]
...
SAMPLE_CLIENTS = [
    ...
    (5, "object", "ООО ТехноСервис", "info@technoserv.ru", "ул. Ленина, 10", "Серверная"),
...
def create_works_database(file_path=""):
    """
    Create SQLite database with completed works and sample rows.
    ...
    with _connect(db_path) as conn:
        ...
        conn.commit()
    return str(db_path), len(SAMPLE_WORKS)
...
def save_services(db_path, services):
    ...
    with _connect(path) as conn:
        ...
        conn.executescript(SERVICES_SCHEMA)
        if services:
            rows = [(item["id"], item["name"].strip(), float(item["price"])) for item in services]
            conn.executemany(
                "INSERT INTO services (id, name, price) VALUES (?, ?, ?)",
                rows,
            )
...
def _migrate_works_table(conn):
    conn.executescript(WORKS_SCHEMA)
    columns = [row[1] for row in conn.execute("PRAGMA table_info(completed_works)").fetchall()]
